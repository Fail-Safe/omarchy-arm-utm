...
candidate** (3.7 GB, `sha256 2671e66c63680f6d…`). It adds current reviewed
source pins, enforced 18-tool and browser-policy contracts, CI, and
...
- **18 Omarchy tools built for aarch64** that upstream does not ship for ARM:
...
**All 18 ARM tools are required and verified.** `herdr` comes from the pinned
`omarchy-pkgs` PKGBUILD, which fetches the official Zig 0.15.2 toolchain.
