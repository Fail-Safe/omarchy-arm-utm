...
Este fork también ha construido y validado `omarchy-arm-utm-v3.zip` (3,7 GB,
`sha256 2671e66c63680f6d…`), pero es una **candidata de publicación**: todavía no
...
- **¿Preparar la imagen para repartir?**
  ...
  - **Sí**: renombra el usuario a `omarchy`, borra claves SSH, identidad de git
    e historiales, y genera un `.zip` de ~3,7 GB con su `sha256`.
...
**3,7 GB** de `.zip`. Decir que no a OBS y Pinta ahorra unos 45 minutos: OBS se
...
candidata validada (`sha256 2671e66c63680f6d…`) pendiente de que el mantenedor
