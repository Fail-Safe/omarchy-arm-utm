#!/bin/bash
# Compile the Omarchy tools not published for ARM for aarch64.
# Almost none are incompatible: most are Rust, Go, or Qt/C++ and just need
...
build() {                      # build <source> <package>
  # A single `local` expands everything before assignment: $pkg would not exist yet.
  local src="$1" pkg="$2"
  local dir="$WORK/$pkg"
  log "$pkg  ($src)"
  ...
  # Many PKGBUILDs list only x86_64 because no one has compiled them for ARM.
  # If the code is portable (Rust/Go/C++), it is sufficient to declare the architecture.
  # 'any' can be used without quotes; mixing it with specific architectures is a
  # makepkg error, so it is only patched when it is NOT 'any' and does NOT already include aarch64.
