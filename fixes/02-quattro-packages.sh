...
# waybar). The quattro branch (4.x) running on the VM uses its own shell based
# on quickshell and requests additional packages. Here that delta is installed.
set -uo pipefail
log() { echo ""; echo "==> $*"; }
...
# autostart.conf) are not read by anyone. The settings go in monitors.lua.
cat > ~/.config/hypr/monitors.lua <<'LUA'
-- Adjusted for VM (UTM/QEMU virtio-gpu).
-- Omarchy assumes 2x Retina displays; in the VM this makes everything huge.
-- Check available resolutions: hyprctl monitors
o.env("GDK_SCALE", "1")
